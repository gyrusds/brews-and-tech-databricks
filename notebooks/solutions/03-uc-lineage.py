...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=governance&org_id=8271961813636051&notebook=%2F00-UC-lineage&demo_name=uc-03-data-lineage&event=VIEW&path=%2F_dbdemos%2Fgovernance%2Fuc-03-data-lineage%2F00-UC-lineage&version=1">

# COMMAND ----------

# MAGIC %md 
# MAGIC ### A cluster has been created for this demo
# MAGIC To run this demo, just select the cluster `dbdemos-uc-03-data-lineage-alvaro_perez` from the dropdown menu ([open cluster configuration](https://adb-8271961813636051.11.azuredatabricks.net/#setting/clusters/0513-180438-fy23ohkt/configuration)). <br />
...
f"""CREATE TABLE IF NOT EXISTS {catalog}.menu (recipe_id INT, app string, main string, desert string);
DELETE from {catalog}.menu ;"""

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS uc_acl.menu (recipe_id INT, app string, main string, desert string);
# MAGIC
# MAGIC DELETE from uc_acl.menu ;
# MAGIC
# MAGIC
# MAGIC INSERT INTO uc_acl.menu 
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS uc_acl.dinner 
# MAGIC   AS SELECT recipe_id, concat(app," + ", main," + ",desert) as full_menu FROM uc_acl.menu
...
df.write.mode("overwrite").saveAsTable("uc_acl.price")

dinner = spark.read.table("uc_acl.dinner")
price = spark.read.table("uc_acl.price")

dinner_price = dinner.join(price, on="recipe_id")
dinner_price.write.mode("overwrite").saveAsTable("uc_acl.dinner_price")
